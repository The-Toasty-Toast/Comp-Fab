...
def gcode_move(current_pos, next_pos, feed_rate=None, should_extrude=False):
 ...
 #____My stuff__    
    # useful from slides 
    #Lout = sqrt((x1 - x0)2 + (y1 - y0)2 + (z1 - z0)2)
    #Aout = Arect + Acircle
    #Arect = h * (w - h)
    #Acircle = π * r2 = π * (h/2)2
    #LE = (Aout * Lout) / (π * (Dfillament/2)**2)

    if (should_extrude):
	    segment_dist = math.sqrt((next_pos[0] - current_pos[0])**2 + (next_pos[1] - current_pos[1])**2 + (next_pos[2] - current_pos[2])**2)
	    area_circle = math.pi * (layer_height/ 2)**2
	    area_rectangle = layer_height * (extrusion_width - layer_height)
	    volume = (area_circle + area_rectangle) * segment_dist
	    filament_segment_length = volume / (math.pi * (filament_diameter/2)**2)
	    move_command_str += " " + PARAM_E + e_value_to_string(filament_segment_length)
    #________________
...
#_______________
        if not is_same_pt(ptA = current_position, ptB = line_start_position):
        ...
        if not is_same_pt(current_position, line_start_position):
            ...
            move_to_line_start_command = gcode_move(current_position, line_start_position, feed_rate=travel_feed_rate, should_extrude=False)
            # A ppend command
